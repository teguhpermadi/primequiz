...
import { index, store } from '@/actions/App/Http/Controllers/GradeController';
        ...
        router.post(store().url, initialValues.value, {
            ...
            onSuccess: () => {
                toast.add({
                    severity: 'success',
                    summary: 'Successful',
                    detail: 'Grade ' + initialValues.value.name + ' Created',
                    life: 3000,
                });
            },
            onError: (error) => {
...
<template>
    <AppLayout>
        <InertiaHead title="Create Grade" />
        <Card>
            <template #title>Create Grade</template>
            <template #content>
